...
try {
    simTools = gb.method.require(gb.modulesPath + '/futureGbSimTools_0-2-0')(gb);
} catch (e) {
...
const ATR_MULTIPLIER_ENTRY = 0.2;      // multiplicateur ATR pour signal d'entrée
const ATR_MULTIPLIER_TP    = 1;      // multiplicateur ATR pour Take Profit
const ATR_MULTIPLIER_SL    = 2;    // multiplicateur ATR pour Stop Loss
...
// Propriétés exigeant un appel à futureGbSimTools dans le mode simulation
if (SIMULATION) {
    const side      = simTools.simCurrentSide;
    const qty       = simTools.simCurrentQty;
    const margin    = simTotalPositionInitialMargin;
} else {
...
// Affichage console des nouvelles valeurs des propriétés futures
